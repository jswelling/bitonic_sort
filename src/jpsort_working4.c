			...
			int (*compar)(const void*, const void*))
{
  ...
  if (nChanges>2) fprintf(stderr,"rank %d: not bitonic! nChanges= %d\n",
			  _gblRank, nChanges);
  else fprintf(stderr,"rank %d: bitonic; dir= %d, nChanges= %d\n",
	       _gblRank,dir,nChanges);
}
#endif
	    ...
	    int (*compar)(const void*, const void*)) 
{
  ...
  se_push(1, OP_SORT, ASCENDING, &stack);
  if (nLevels>1) {
    se_push(nLevels, OP_MERGE, ASCENDING, &stack);
    se_push(nLevels/2, OP_SORT, 
	    (myRank+nGhosts>=nLevels/2?DESCENDING:ASCENDING), &stack);
  }
  TIMECOUNT(nDummy, dummyTime);
  while (stack) {
    int lvl= stack->lvl;
	...
	if (dir==ASCENDING) {
      ...
      if (lvl>2) se_push(lvl/2,OP_MERGE,dir,&stack);
      break;
    }
#ifdef DEBUG
